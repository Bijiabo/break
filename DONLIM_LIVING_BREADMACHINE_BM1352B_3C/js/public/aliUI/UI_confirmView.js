...
define(['./../vue', './public'], function(Vue, _public){
    ...
    var initConfirmView = function(component) {
        ...
        if (component.$get('itemData.cancel')) {
            buttonConfig.unshift({
                ...
                click:function(){
                    var itemData = component.$get('itemData');
                    if (itemData.cancel.customTapFunction) {
                        // 执行自定义函数
                        itemData.cancel.customTapFunction(component.$get('data'), component);
                    } else {
    ...
    var ConfirmView = Vue.extend({
        template: '<div :class="[show ? \'\' : \'hide\' ]" :vv="value">\
                        <div class="ui-modal upt-ui-modal confirm-view_{{index}} {{customClass}}"></div>\
                    </div>',
        data: function() {
            return {
                show: true,
        ...
        ready: function() {
            ...
            this.customClass = this.itemData.customClass || '';
        },
        watch: {
            display: {
